 ...
 *
 */
public class DefaultFitnessFunction3 implements FitnessFunction {
...
//	uniform distribution
	...
	public DefaultFitnessFunction3(Parameters p){
		this.p = p;
	}
	/* (non-Javadoc)
...
//		number of genes in a network
		...
		for (int i = 0; i < pop.size(); i++){
			avgSinks += pop.get(i).getSinks().size();
		}
		avgSinks /= pop.size();
...
//			get network's regulators
			...
			genes = net.getGeneIndices().size()-sinks;
//			calculating fitness
...
//				System.out.println("gt_term: " +gt_term);
				...
				genes_term = (1 / (1 + Math.abs(p.OPTIMAL_GENE_NUMBER - sinks)));
