...
//				 for each input...copy it with certain probability
				if (uniform.nextDouble() <= p.KEEP_INPUT_PROBABILITY)
					net.addInteraction(input.getElement(), input.getValue(), newgene);
...
//				 for each regulatory output...copy it with certain probability
				if (uniform.nextDouble() <= p.KEEP_OUTPUT_PROBABILITY)
					net.addInteraction(newgene, output.getValue(), output.getElement());
...
//							for each regulatory output...copy it with certain probability
							if (uniform.nextDouble() <= p.KEEP_DIMER_OUTPUT_PROBABILITY)
								net.addInteraction(newhomodimer, output.getValue(), output.getElement());
						}
...
//							for each regulatory output...copy it with certain probability
							if (uniform.nextDouble() <= p.KEEP_DIMER_OUTPUT_PROBABILITY)
								net.addInteraction(newheterodimer, output.getValue(), output.getElement());
						}
...
//		get indices of all regulators (genes or dimers that have regulatory outputs)
		ElementVector regulators = net.getRegulators();
...
//			pick random element (all elements can potentially have regulatory output)
			Element element = this.pickRandomElement(net);
...
//		operation unsuccessful
	 ...
	 * @return if successful, this method returns <tt>true</tt>, otherwise <tt>false</tt>.
	 */
	protected synchronized boolean formNewHomodimer(Network net){
...
//			this will allow for the new dimer partner to form another dimer already
			Element geneOfNewDimer = this.pickRandomGene(net);
...
//					perform gene duplications
					if (genedynamicsrate <= p.GENE_DUPLICATION_RATE){ 
						...
						if(p.VERBOSE) System.out.println("duplicateGenesPoisson(net)"+n);
						net.getDataCollector().addToMutationHistory(10+n);
					}
//					perform whole genome duplication
					if (genedynamicsrate > p.GENE_DUPLICATION_RATE 
							&& genedynamicsrate <= p.GENOME_DUPLICATION_RATE + p.GENE_DUPLICATION_RATE){
						duplicateGenome(net);
						if(p.VERBOSE) System.out.println("duplicateGenome(net)!!");
						net.getDataCollector().addToMutationHistory(10+net.size());
					}
//					perform gene deletions
					if (genedynamicsrate > p.GENE_DUPLICATION_RATE + p.GENOME_DUPLICATION_RATE 
						...
						if(p.VERBOSE) System.out.println("deleteGenesPoisson(net)"+n);
						net.getDataCollector().addToMutationHistory(-(10+n));
					}
...
//					substitute one input with another
					if (linkdynamicsrate <= p.INPUT_CHANGE_RATE){ 
						...
						if(p.VERBOSE) System.out.println("changeInputOfRandomGene(net)"+success);
						net.getDataCollector().addToMutationHistory(1);
					}
//					substitute one output with another
					if (linkdynamicsrate > p.INPUT_CHANGE_RATE 
						...
						if(p.VERBOSE) System.out.println("changeOutputOfRandomRegulator(net)"+success);
						net.getDataCollector().addToMutationHistory(2);
					}
//					remove one regulatory interaction
					if (linkdynamicsrate > p.INPUT_CHANGE_RATE + p.OUTPUT_CHANGE_RATE
						...
						if(p.VERBOSE) System.out.println("deleteRandomRegulatoryInteraction(net)"+success);
						net.getDataCollector().addToMutationHistory(3);
					}
//					form one new regulatory interaction
					if (linkdynamicsrate > p.INPUT_CHANGE_RATE + p.OUTPUT_CHANGE_RATE + p.LINK_DELETION_RATE
						...
						if(p.VERBOSE) System.out.println("formNewRandomRegulatoryInteraction(net)"+success);
						net.getDataCollector().addToMutationHistory(4);
					}
//					switch link between activation and inhibition
					if (linkdynamicsrate > p.INPUT_CHANGE_RATE + p.OUTPUT_CHANGE_RATE + p.LINK_DELETION_RATE + p.LINK_FORMATION_RATE
						...
						if(p.VERBOSE) System.out.println("swapRegulatorySign(net)"+success);
						net.getDataCollector().addToMutationHistory(5);
					}
				}
...
//					form completely new homodimer (= new dimerising domain)
					if (dimerdynamicsrate <= p.NEW_HOMODIMER_RATE){
						...
						if(p.VERBOSE) System.out.println("formNewHomodimer(net)"+success);
						net.getDataCollector().addToMutationHistory(6);
					}
//					form new dimer from existing dimer (= spread of dimerising domain)
					if (dimerdynamicsrate > p.NEW_HOMODIMER_RATE
						...
						if(p.VERBOSE) System.out.println("formNewHeterodimer(net)"+success);
						net.getDataCollector().addToMutationHistory(7);
					}
//					remove one dimer (loss of dimerising domain in transcription factor gene)
					if (dimerdynamicsrate > p.NEW_HOMODIMER_RATE + p.NEW_HETERODIMER_RATE
						...
						if(p.VERBOSE) System.out.println("removeRandomDimer(net)"+success);
						net.getDataCollector().addToMutationHistory(8);
					}
