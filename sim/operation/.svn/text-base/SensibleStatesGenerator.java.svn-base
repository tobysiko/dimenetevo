...
public class SensibleStatesGenerator extends AbstractStatesGenerator implements Serializable{
	...
	private ElementVector consideredElements;
	/**
	 * 
	 */
...
//						dummy.removeElement(e);
	...
	private void removeSinks(Network net, MyVector<Integer> toBeRemoved, Element e, int count){
		if (! (count == 0)) {
...
//						dummy.removeElement(e);
	...
	public boolean[] getNotConsideredElements(){
		...
		for (int i = 0; i < notConsidered.length; i++) {
			Element e = elements.get(i);
			if (isolated.contains(e) || sinks.contains(e) || dimers.contains(e)) {
				notConsidered[i] = true;
			}			
		}		
		return notConsidered;
	...
	private void permutate(int n, boolean[] notConsidered){
...
//			allStates.remove(0);
	...
	private void recPermutate(int n, int c, boolean[] bool, boolean[] notConsidered){
