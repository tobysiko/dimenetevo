...
//		the index of rankedIndices from which on networks are not considered to be 'fit' anymore
		int fit_unfit_boundary = (int)Math.ceil(rankedIndices.length * p.FITTEST_PROPORTION);
...
//		mutate new population
		if (p.PRINT_DURATIONS){
			long preMute = new Date().getTime();
			mut.mutatePopulation(p, newPop);
			long postMute = new Date().getTime();
			System.out.println("\tmutatePopulation(p, newPop): " +(postMute-preMute) +"ms");
		}else
			mut.mutatePopulation(p, newPop);
