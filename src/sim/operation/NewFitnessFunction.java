...
public class NewFitnessFunction implements FitnessFunction{
...
//	uniform distribution
	...
	public NewFitnessFunction(Parameters p){
		this.p = p;
	}
...
//				if (genes < p.MAX_PICKS_POWER)
//					gt_term = (1 / (1 + Math.sqrt((Math.pow(2, genes)) - attractors)));
//				else
//					gt_term = (1 / (1 + Math.sqrt((Math.pow(2, p.MAX_PICKS_POWER)) - attractors)));
				
				gt_term = (1 / (1 + Math.abs(p.OPTIMAL_GT_RATIO - gt)));  
//				gt_term = (1 / (1 + Math.pow(Math.E, Math.abs(p.OPTIMAL_GT_RATIO - gt))));  
...
//				System.out.println("subgraph_term: "+subgraph_term);
				...
				popFitness[i] = (gt_term + genes_term) * subgraph_term;
