...
public class DefaultSimulator implements Simulator {
	 ...
	 */
	protected synchronized boolean[] updateSynchronously(Network net, boolean[] previousStep) {
...
//				 for each element ...
//				update genes/monomers first so that dimers will be on in the same timestep
	 ...
	 */
	public synchronized Attractor findAttractor(boolean[] state) {
	 ...
	 */
	public synchronized Attractor simulateSingleState(Network net,
	 ...
	 */
	public synchronized void simulateAllStates(Network net) {
		if (p.MINIMAL_OUTPUT)
			System.out
...
////		can I do this ??
			net.getDataCollector().removeAttractors();//to save memory....
...
//		ElementVector elements = net.getElementVector();
	...
	protected void removeSinks(Network net,ElementVector consideredElements, Element e, int count){
		if (! (count == 0)) {
